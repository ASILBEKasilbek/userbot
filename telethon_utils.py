...
from db import load_groups, save_group, remove_group, get_setting
...
async def auto_reply_handler(event):
    ...
    auto_reply_enabled = bool(int(get_setting("auto_reply_enabled") or 0))
    auto_reply_text = get_setting("auto_reply_text") or "Salom! Bu avtomatik javob."
...
async def response_reply_handler(event):
    ...
    response_reply_enabled = bool(int(get_setting("response_reply_enabled") or 0))
    response_reply_text = get_setting("response_reply_text") or "Avto javob guruhda."
...
async def leave_group(client: TelegramClient, group_id: int, profile_id: int):
    """Guruhdan chiqish va uni ma'lumotlar bazasidan o‘chirish."""
    try:
        await client(LeaveChannelRequest(group_id))
        logger.info(f"{client._self_id} guruhdan chiqildi: {group_id}")
    except Exception as e:
        logger.error(f"{client._self_id} guruhdan chiqishda xato: {e}")
    ...
    try:
        if isinstance(entity, Channel):
            ...
            if full.full_chat.linked_chat_id:
                ...
                save_group(link, profile_id)
                logger.info(f"{client._self_id} linked kanalga qo‘shildi: {link}")
                return True
    except Exception as e:
        logger.error(f"{client._self_id} linked kanalni tekshirishda xato: {e}")
    return False
...
async def send_to_groups_auto(clients: list):
    ...
    message_text = get_setting("message_text") or "📢 Bu avtomatik xabar!"
    messages_per_minute = int(get_setting("messages_per_minute") or 30)
    send_interval = int(get_setting("send_interval") or 60)
    while bool(int(get_setting("auto_send_enabled") or 0)):
        for client in clients:
            ...
            for link in groups.copy():
                try:
                    ...
                    logger.warning(f"{client._self_id} yozish taqiqlangan: {link} — Linked kanal tekshirilmoqda...")
                    if await handle_linked_channel(client, entity, profile_id):
                        ...
                        await leave_group(client, entity.id, profile_id)
                except ChannelPrivateError:
                    logger.warning(f"{client._self_id} maxfiy kanal: {link}")
                    remove_group(link, profile_id)
                    await leave_group(client, entity.id, profile_id)
    ...
    logger.info("Avtomatik yuborish to‘xtatildi.")