...
from db import load_profiles, save_profile, remove_profile, load_groups, get_setting, set_setting
...
from constants import MESSAGES, MAIN_MENU_BUTTONS, PROFILE_MENU_BUTTONS
from keyboards import get_main_keyboard, get_profile_keyboard, get_profile_selection_keyboard
from telethon.errors import SessionPasswordNeededError, PhoneNumberInvalidError, ApiIdInvalidError
...
@dp.message(MainForm.main_menu, F.text == MAIN_MENU_BUTTONS["TOGGLE_AUTO_SEND"])
@admin_only
async def toggle_auto_send(message: types.Message, state: FSMContext):
    auto_send_enabled = bool(int(get_setting("auto_send_enabled") or 0))
    auto_send_enabled = not auto_send_enabled
    set_setting("auto_send_enabled", "1" if auto_send_enabled else "0")
    status = "boshlandi" if auto_send_enabled else "to'xtatildi"
    await message.answer(MESSAGES["AUTO_SEND_TOGGLED"].format(status=status), reply_markup=get_main_keyboard())
...
@dp.message(MainForm.main_menu, F.text == MAIN_MENU_BUTTONS["DELETE_PROFILE"])
@admin_only
async def delete_profile_start(message: types.Message, state: FSMContext):
    ...
    await message.answer("🗑 O'chiriladigan profilni tanlang:", reply_markup=get_profile_selection_keyboard(profiles))

@dp.message(MainForm.waiting_for_profile_delete, F.text.regexp(r'\+998\d{9}'))
@admin_only
async def process_delete_profile(message: types.Message, state: FSMContext):
    ...
    if not selected:
        await message.answer(MESSAGES["PROFILE_NOT_FOUND"].format(phone=phone))
        return
    client = next((c for c in clients if c.profile_id == selected['id']), None)
    if client:
        try:
            await client.disconnect()
    ...
    remove_profile(selected['id'])
    await message.answer(MESSAGES["PROFILE_DELETED"].format(phone=phone))
    await state.set_state(MainForm.main_menu)
    await message.answer(MESSAGES["BACK_TO_MAIN"], reply_markup=get_main_keyboard())

@dp.message(MainForm.main_menu, F.text == MAIN_MENU_BUTTONS["LIST_GROUPS"])
@admin_only
async def show_all_groups(message: types.Message, state: FSMContext):
    profiles = load_profiles()
    all_groups = set()
    for profile in profiles:
        groups = load_groups(profile['id'])
        all_groups.update(groups)
    if not all_groups:
        await message.answer(MESSAGES["NO_GROUPS"])
        return
    text = "\n".join([f"{i+1}. {g}" for i, g in enumerate(all_groups)])
    await message.answer(f"📋 Barcha guruhlar:\n{text}", reply_markup=get_main_keyboard())
    ...
    for link in links:
        if link not in groups:
            try:
                ...
                await client.send_message(entity, get_setting("message_text"))
                added += 1
            except Exception as e:
...
async def profile_info(message: types.Message, state: FSMContext):
    ...
    info = (
        ...
        f"🔄 Avto javob: {'Faol' if bool(int(get_setting('auto_reply_enabled'))) else 'O‘chirilgan'}\n"
        f"📝 Avto javob matni: {get_setting('auto_reply_text')}\n"
        f"🔄 Guruh avto javobi: {'Faol' if bool(int(get_setting('response_reply_enabled'))) else 'O‘chirilgan'}\n"
        f"📝 Guruh avto javob matni: {get_setting('response_reply_text')}\n"
        f"✉ Yuboriladigan xabar: {get_setting('message_text')}\n"
        f"⏱ 1 daqiqada guruhlar: {get_setting('messages_per_minute')}\n"
        f"⏰ Yuborish oralig'i: {int(get_setting('send_interval')) // 60} daqiqa\n"
        f"🚀 Avtomatik yuborish: {'Faol' if bool(int(get_setting('auto_send_enabled'))) else 'O‘chirilgan'}"
    )
    await message.answer(info, reply_markup=get_profile_keyboard())

@dp.message(MainForm.main_menu, F.text == MAIN_MENU_BUTTONS["CHANGE_AUTO_REPLY_TEXT"])
@admin_only
...
@dp.message(MainForm.main_menu, F.text == MAIN_MENU_BUTTONS["CHANGE_RESPONSE_REPLY_TEXT"])
@admin_only
...
@dp.message(MainForm.main_menu, F.text == MAIN_MENU_BUTTONS["MESSAGE_TEXT"])
@admin_only
